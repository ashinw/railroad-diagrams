...
class Container extends Component {
    ...
    prepareItemsPriorToLinage(items) {
        for (let i = items.length - 1; i >= 0; i--) {
            if (!items[i]) // ie. null or undefined or empty string
                items.splice(i, 1);
        }
    }
    prepareComponentLinage(items) {
    ...
    assertValidLinageVerification(childCtx) {
...
class SequenceableContainer extends Container {
    constructor(items, tagName, attrs, text) {
    ...
    formatSequentially(target, x, y, width) {
    ...
    renderComponent(item, x, y, target) {
        item.format(x, y, item.width).addTo(target);
    }
    formatDownlineConnection(x, item, y, i, target) {
        x += item.width;
        y += item.height;
    ...
    formatUplineConnection(item, i, x, y, target) {
    ...
    formatEntryExits(width, x, y, target) {
...
export class TrackDiagram extends SequenceableContainer {
    constructor(...items) {
...
export class Group extends Container {
    constructor(...items) {
        ...
        this.width = Math.max(this.item.width + (this.item.needsSpace ? 20 : 0), this.label ? this.label.width : 0, Options.AR * 2);
        this.height = this.item.height;
...
class LabelTitleLinkControl extends Control {
    ...
    formatSides(x, y, width, addToRightY = 0) {
        // Hook up the two sides if this is narrower than its stated width.
        var gaps = determineGaps(width, this.width);
        if (this.canConnectUpline()) {
            new Path(x, y).h(gaps[0]).addTo(this);
...
function makeTerminalIfString(value) {
